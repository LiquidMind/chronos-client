...
def check_network_connection(server="www.google.com"):
    """
    Checks if Chronos can connect a network server.
...
def get_pip_requirements(fname=os.path.join(chronospath.LIB_PATH,
                                            'requirements.txt')):
    """
...
def run():
    ...
    logger.info("Starting chronos diagnostic at %s" % time.strftime("%c"))
    ...
    for fname in [os.path.join(chronospath.APP_PATH, os.pardir, "phonetisaurus",
                               "g014b2b.fst")]:
